"""ShuffleNet for CIFAR10/100

Reference:
Zhang, Xiangyu, et al. ShuffleNet: An Extremely Efficient Convolutional Neural Network for Mobile Devices (CVPR '18)
"""
...
class DepthwiseConv2d(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, **kwargs):
        ...
        self.depthwise = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, kernel_size, **kwargs), nn.BatchNorm2d(output_channels)
        )
...
class ShuffleNetUnit(nn.Module):
    def __init__(self, input_channels, output_channels, stage, stride, groups):
        ...
        self.depthwise = DepthwiseConv2d(
            int(output_channels / 4), int(output_channels / 4), 3, groups=int(output_channels / 4), stride=stride, padding=1
        )
...
class ShuffleNet(nn.Module):
    def __init__(self, dataset, num_blocks, groups=3):
        super().__init__()
...
def shufflenet(Dataset):
    return ShuffleNet(Dataset, [4, 8, 4])
