"""NASNet for CIFAR10/100

Reference:
Zoph, Barret, et al. Learning Transferable Architectures for Scalable Image Recognition (CVPR '18)
"""
...
class SeperableBranch(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, **kwargs):
        ...
        self.block1 = nn.Sequential(
            nn.ReLU(), SeperableConv2d(input_channels, output_channels, kernel_size, **kwargs), nn.BatchNorm2d(output_channels)
        )
...
class NormalCell(nn.Module):
    def __init__(self, x_in, prev_in, output_channels):
        ...
        self.dem_reduce = nn.Sequential(
            nn.ReLU(), nn.Conv2d(x_in, output_channels, 1, bias=False), nn.BatchNorm2d(output_channels)
        )
...
class NasNetA(nn.Module):
    def __init__(self, dataset, repeat_cell_num, reduction_num, filters, stemfilter):
        super().__init__()
...
def nasnet(Dataset):
    return NasNetA(Dataset, 4, 2, 44, 44)
