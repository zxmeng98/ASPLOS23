"""InceptionV4 for CIFAR10/100

Reference:
Szegedy, Christian, et al. Inception-v4, Inception-ResNet and the Impact of Residual Connections on Learning (AAAI '17)
"""
...
class InceptionA(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branch3x3 = nn.Sequential(
            BasicConv2d(input_channels, 64, kernel_size=1), BasicConv2d(64, 96, kernel_size=3, padding=1)
        )

        self.branch1x1 = BasicConv2d(input_channels, 96, kernel_size=1)

        self.branchpool = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=1, padding=1), BasicConv2d(input_channels, 96, kernel_size=1)
        )
...
class ReductionB(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branch3x3 = nn.Sequential(
            BasicConv2d(input_channels, 192, kernel_size=1), BasicConv2d(192, 192, kernel_size=3, stride=2, padding=1)
        )
...
class InceptionC(nn.Module):
    def __init__(self, input_channels):
        ...
        self.branchpool = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=1, padding=1), BasicConv2d(input_channels, 256, kernel_size=1)
        )
...
class InceptionV4(nn.Module):
    def __init__(self, dataset, A, B, C, k=192, l=224, m=256, n=384):
...
class InceptionResNetA(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branch3x3 = nn.Sequential(
            BasicConv2d(input_channels, 32, kernel_size=1), BasicConv2d(32, 32, kernel_size=3, padding=1)
        )
...
class InceptionResNetReductionB(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branch3x3a = nn.Sequential(
            BasicConv2d(input_channels, 256, kernel_size=1), BasicConv2d(256, 384, kernel_size=3, stride=2)
        )

        self.branch3x3b = nn.Sequential(
            BasicConv2d(input_channels, 256, kernel_size=1), BasicConv2d(256, 288, kernel_size=3, stride=2)
        )
...
class InceptionResNetV2(nn.Module):
    def __init__(self, dataset, A, B, C, k=256, l=256, m=384, n=384):
        super().__init__()
...
def inceptionv4(Dataset):
    return InceptionV4(Dataset, 4, 7, 3)


def inception_resnet_v2(Dataset):
    return InceptionResNetV2(Dataset, 5, 10, 5)
